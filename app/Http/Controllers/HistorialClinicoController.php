...
class HistorialClinicoController extends Controller
{
	public function index(Request $request)
	{
		...
		{
			if ($buscar==''){
				$historialClinico = HistorialClinico::with('clinico')->with('paciente')->orderBy('id', 'desc')->where('estado',1)->paginate($count);
			}
			else{
				$historialClinico = HistorialClinico::with('clinico')->with('paciente')->where([[$criterio, 'like',$buscar],['estado',1]])->orderBy('id', 'desc')->paginate($count);
			}
		} else if ($completo == 'true'){
			if ($buscar==''){
				$historialClinico = HistorialClinico::with('clinico')->with('paciente')->orderBy('id', 'desc')->paginate($count);
			}
			else{
				$historialClinico = HistorialClinico::with('clinico')->with('paciente')->where($criterio,'like',$buscar)->orderBy('id', 'desc')->paginate($count);
			}
		}
		return [
