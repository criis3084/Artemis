...
class HistorialExamenController extends Controller
{
	public function index(Request $request)
	{
		...
		{
			if ($buscar==''){
				$historialExamen = HistorialExamen::with('examen')->with('paciente')->orderBy('id', 'desc')->where('estado',1)->paginate($count);
			}
			else{
				$historialExamen = HistorialExamen::with('examen')->with('paciente')->where([[$criterio, 'like',$buscar],['estado',1]])->orderBy('id', 'desc')->paginate($count);
			}
		} else if ($completo == 'true'){
			if ($buscar==''){
				$historialExamen = HistorialExamen::with('examen')->with('paciente')->orderBy('id', 'desc')->paginate($count);
			}
			else{
				$historialExamen = HistorialExamen::with('examen')->with('paciente')->where($criterio,'like',$buscar)->orderBy('id', 'desc')->paginate($count);
			}
		}
		return [
