	...
	<div>
        ...
        <div class="flex">
          ...
          <vs-switch v-model="cellAutoWidth">Cell Auto Width</vs-switch>
			...
			<vs-table stripe title="Constructores" pagination max-items="7" search :data="arrayData" noDataText="No hay datos disponibles">
	   <template slot="header">
          <vs-button @click="activePrompt=true">Exportar</vs-button>
				...
				<vs-prompt
					:buttons-hidden="true"
					title="Listado de construcciones"
					:active.sync="abrirListado">
						<li v-for="(vivienda,index) in listadoConstrucciones" v-bind:key="index">
							{{ vivienda }}
						</li>
				</vs-prompt>
		</vx-card>
...
export default {
  data () {
    return {
      ...
      cellAutoWidth: true,
	  selectedFormat: 'xlsx',
	  headerVal: ['id', 'nombres', 'apellidos', 'numero_telefono'],
	  headerTitle: ['Id', 'Nombre', 'Apellidos', 'Telefono']
    }
  },
  components: {
    VueApexCharts,
  ...
  methods: {
    ...
    cambiarEstado (color) {
		...
		axios.put('/api/constructor/activar', {
			id: this.id
		})
			.then(function (response) {
			...
			axios.put('/api/constructor/desactivar', {
			id: this.id
		})
		.then(function (response) {
			console.log(response.data.message)
    ...
    aNuevo () {
		 this.$router.push('/ingresar/constructor')
	},
	exportToExcel () {
