<template>
	<div>
		<vx-card>
			...
			<vs-table stripe pagination max-items="7" search :data="arrayData" oDataText="No hay datos disponibles" >
...
import ingresarsalida from "./ingresarsalida.vue";
  ...
  data() {
	...
	methods: {
		...
		cambiarEstado(color) {
			...
			axios
				.put("/api/salidaMedicamento/activar", {
				id: this.id
				})
			...
			axios
				.put("/api/salidaMedicamento/desactivar", {
				id: this.id
				})
				.then(function(response) {
				console.log(response.data.message);
		...
		async index() {
			...
			.get(`/api/salidaMedicamento/get?completo=true`)
			.then(function(response) {
				var respuesta = response.data;
				me.arrayData = respuesta.salidaMedicamentos.data;
 		...
 		aNuevo () {
		 this.$router.push('/ingresar/salidamedi')
		},	
		exportToExcel () {
