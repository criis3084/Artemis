	...
	<div>
		<vx-card>
			...
			<vs-table stripe pagination max-items="7" search :data="arrayData" noDataText="No hay datos disponibles" >
				...
				<template slot="thead">
					<vs-th>Id</vs-th>
					<vs-th>Medicamento</vs-th>
					<vs-th>Descripción</vs-th>
					<vs-th>Stock</vs-th>
					<vs-th>Categoría</vs-th>
					<vs-th>Casa Médica</vs-th>
					<vs-th>Estado</vs-th>
				...
				<template slot-scope="{ data }">
					<vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
						...
						<vs-td>{{ data[indextr].stock_general }}</vs-td>
                        <vs-td>{{ data[indextr].nombre_categoria }}</vs-td>
                        <vs-td>{{ data[indextr].nombre_casa }}</vs-td>
			...
			<editar
				...
				v-bind:stock_general="stock_general"
				v-bind:categoria_medicamento_id="categoria_medicamento_id"
				v-bind:casa_medica_id="casa_medica_id"
				v-on:cerrado="index();"
			></editar>
		</vx-card>
	</div>
...
export default {
  data() {
		return {
			...
			cellAutoWidth: true,
			selectedFormat: 'xlsx',
			headerVal: ['id', 'nombre', 'descripcion', 'stock_general', 'nombre_categoria', 'nombre_casa', 'estado' ],
			headerTitle: ['Id', 'Nombre del Medicamento', 'Descripción', 'Stock General', 'Categoría del Medicamento', 'Casa Médica' ,'Estado'],
			activePrompt: false,
			nombre: "",
			descripcion: "",
			stock_general:0,
			switch2: false,
			categoria_medicamento_id:0,
			casa_medica_id:0,
			abrir_editar:false,
			id: 0,
			estado: null
	...
	methods: {
		traerDatos(tabla){
			tabla.forEach(function(valor, indice, array){
                valor.nombre_categoria=valor.categoria.nombre
                valor.nombre_casa=valor.casa_medica.nombre
			}); 
			return tabla
		},
		cambiar(medicamento){
			...
			this.stock_general = medicamento.stock_general;
			this.categoria_medicamento_id = medicamento.categoria_medicamento_id;
			this.casa_medica_id = medicamento.casa_medica_id;
