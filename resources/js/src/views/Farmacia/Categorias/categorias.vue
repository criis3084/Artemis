...
export default {
  ...
  methods: {
	  cambiar(categoriaMedicamento){
		  this.id = categoriaMedicamento.id;
          this.nombre = categoriaMedicamento.nombre;
          this.descripcion = categoriaMedicamento.descripcion;
	...
	cambiarEstado(color){
		...
		if(this.estado === 0 || this.estado === false){
			...
			axios.put('/api/categoriaMedicamento/activar', {
				id: this.id
			})
			...
			axios.put('/api/categoriaMedicamento/desactivar', {
				id: this.id
			})
			.then(function (response) {
				console.log(response.data.message)
	...
	async index(page, search){ //async para que se llame cada vez que se necesite
		...
		const response = await axios.get(
			`/api/categoriaMedicamento/get?completo=true`)
		.then(function (response) {
			var respuesta= response.data;
			me.arrayData = respuesta.categoriaMedicamentos.data;
