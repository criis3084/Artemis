<template>
	<div>
		<vx-card>
			...
			<vs-table stripe title="Tipos de Examenes" pagination max-items="7" search :data="arrayData" noDataText="No hay datos disponibles">
  ...
  methods: {
	...
	cambiarEstado(color){
		...
		if(this.estado === 0 || this.estado === false){
			...
			axios.put('/api/tipoExamen/activar', {
				id: this.id
			})
			...
			axios.put('/api/tipoExamen/desactivar', {
				id: this.id
			})
			.then(function (response) {
				console.log(response.data.message)
	...
	async index(){
		...
		const response = await axios.get(
			`/api/tipoExamen/get?completo=true`)
		.then(function (response) {
			var respuesta= response.data;
			me.arrayData = respuesta.tipoExamenes.data;
		})
		.catch(function (error) {
			console.log(error);
		});
